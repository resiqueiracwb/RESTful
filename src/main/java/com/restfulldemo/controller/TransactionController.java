...
public class TransactionController {
	 ...
	 * @since 11/02/2020
	 * 
	 * @param transaction, where: id - transaction id; nsu - identification number
	 *                     of a sales transaction using cards. May be null if
	 *                     transaction was paid in cash; autorizationNumber - is a
	 *                     one-time code used in the processing of online
	 *                     transactions; amount – transaction amount; a string of
	 *                     arbitrary length that is parsable as a BigDecimal;
	 *                     transactionDate – transaction time in the ISO 8601 format
	 *                     YYYY-MM-DDThh:mm:ss.sssZ in the Local timezone; type -
	 *                     transaction type: CARD (credit-card) or MONEY (paid in
	 *                     cash).
	 * 
	 * @return Returns an empty body with one of the following: 201 – in case of
	 *         success 400 – if the JSON is invalid 422 – if any of the fields are
	 *         not parsable or the transaction date is in the future
	 * 
	...
	public ResponseEntity<Transaction> create(@RequestBody JSONObject transaction)
			throws InvalidJSONException, NotParsableContentException {
		try {
	...
	public ResponseEntity<Transaction> update(@PathVariable("id") long id, @RequestBody JSONObject transaction)
			throws InvalidJSONException, NotParsableContentException, TransactionNotFoundException {
